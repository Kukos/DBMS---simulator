...
GTEST_TEST(eAdaptiveMerging, interfaceSSD)
{
    ...
    DBIndex* am = new EAdaptiveMerging(index, startingEntries, partitionSize);

    // check how to get some statistics
    EXPECT_EQ(std::string(am->getName()), std::string("ExtendendAdaptiveMerging"));
    ...
    EXPECT_EQ(dynamic_cast<EAdaptiveMerging*>(am)->getMemoryManager().getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<const EAdaptiveMerging::AMPartitionManager&>(dynamic_cast<EAdaptiveMerging*>(am)->getMemoryManager()).getPartitionsRef().size(), (startingEntries + (partitionSize / recordSize) - 1) / (partitionSize / recordSize));
...
GTEST_TEST(eAdaptiveMerging, copySSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    EAdaptiveMerging* copy = new EAdaptiveMerging(*dynamic_cast<EAdaptiveMerging*>(am));

    EXPECT_EQ(std::string(copy->getName()), std::string("ExtendendAdaptiveMerging"));
    ...
    EXPECT_EQ(dynamic_cast<EAdaptiveMerging*>(copy)->getMemoryManager().getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<const EAdaptiveMerging::AMPartitionManager&>(dynamic_cast<EAdaptiveMerging*>(copy)->getMemoryManager()).getPartitionsRef().size(), (startingEntries + (partitionSize / recordSize) - 1) / (partitionSize / recordSize));

    EAdaptiveMerging copy2(*copy);

    EXPECT_EQ(std::string(copy2.getName()), std::string("ExtendendAdaptiveMerging"));
    ...
    EXPECT_EQ(dynamic_cast<EAdaptiveMerging*>(&copy2)->getMemoryManager().getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<const EAdaptiveMerging::AMPartitionManager&>(dynamic_cast<EAdaptiveMerging*>(&copy2)->getMemoryManager()).getPartitionsRef().size(), (startingEntries + (partitionSize / recordSize) - 1) / (partitionSize / recordSize));

    EAdaptiveMerging copy3;
    copy3 = copy2;

    EXPECT_EQ(std::string(copy3.getName()), std::string("ExtendendAdaptiveMerging"));
    ...
    EXPECT_EQ(dynamic_cast<EAdaptiveMerging*>(&copy3)->getMemoryManager().getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<const EAdaptiveMerging::AMPartitionManager&>(dynamic_cast<EAdaptiveMerging*>(&copy3)->getMemoryManager()).getPartitionsRef().size(), (startingEntries + (partitionSize / recordSize) - 1) / (partitionSize / recordSize));
...
GTEST_TEST(eAdaptiveMerging, moveSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    EAdaptiveMerging* copy = new EAdaptiveMerging(std::move(*dynamic_cast<EAdaptiveMerging*>(am)));
    ...
    EAdaptiveMerging copy2(std::move(*copy));
...
GTEST_TEST(eAdaptiveMerging, insertSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(eAdaptiveMerging, bulkloadSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(eAdaptiveMerging, singleFindPointSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    EXPECT_EQ(dynamic_cast<const EAdaptiveMerging::AMPartitionManager&>(dynamic_cast<EAdaptiveMerging*>(am)->getMemoryManager()).getPartitionsRef().size(), numPartitions);
    ...
    EXPECT_EQ(am->getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<EAdaptiveMerging*>(am)->getMemoryManager().getNumEntries(), startingEntries - 1);
...
GTEST_TEST(eAdaptiveMerging, singleFindRangeSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(eAdaptiveMerging, singleBigFindRangeSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    EXPECT_EQ(am->getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<EAdaptiveMerging*>(am)->getMemoryManager().getNumEntries(), startingEntries - numOperations);
    EXPECT_EQ(index->getNumEntries(), numOperations);
...
GTEST_TEST(eAdaptiveMerging, fullFindPointSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    size_t i = 100;
    while (dynamic_cast<EAdaptiveMerging*>(am)->getMemoryManager().getNumEntries() > 0)
    {
        EXPECT_EQ(dynamic_cast<const EAdaptiveMerging::AMPartitionManager&>(dynamic_cast<EAdaptiveMerging*>(am)->getMemoryManager()).getPartitionsRef().size(), numPartitions);
        ...
        EXPECT_EQ(am->getNumEntries(), startingEntries);
        EXPECT_EQ(dynamic_cast<EAdaptiveMerging*>(am)->getMemoryManager().getNumEntries() + index->getNumEntries(), startingEntries);
...
GTEST_TEST(eAdaptiveMerging, fullFindRangeSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(eAdaptiveMerging, singleDeleteSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(eAdaptiveMerging, singleBigDeleteSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(eAdaptiveMerging, interfacePCM)
{
...
GTEST_TEST(eAdaptiveMerging, copyPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, movePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, insertPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, bulkloadPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, singleFindPointPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, singleFindRangePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, singleBigFindRangePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, fullFindPointPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, fullFindRangePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, singleDeletePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(eAdaptiveMerging, singleBigDeletePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
