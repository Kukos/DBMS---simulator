...
GTEST_TEST(fAdaptiveMerging, interfaceSSD)
{
    ...
    EXPECT_EQ(std::string(am->getName()), std::string("FlaggedAdaptiveMerging"));
    ...
    EXPECT_EQ(dynamic_cast<FAdaptiveMerging*>(am)->getMemoryManager().getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<const FAdaptiveMerging::AMPartitionManager&>(dynamic_cast<FAdaptiveMerging*>(am)->getMemoryManager()).getPartitionsRef().size(), (startingEntries + (partitionSize / recordSize) - 1) / (partitionSize / recordSize));
...
GTEST_TEST(fAdaptiveMerging, copySSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    FAdaptiveMerging* copy = new FAdaptiveMerging(*dynamic_cast<FAdaptiveMerging*>(am));

    EXPECT_EQ(std::string(copy->getName()), std::string("FlaggedAdaptiveMerging"));
    ...
    EXPECT_EQ(dynamic_cast<FAdaptiveMerging*>(copy)->getMemoryManager().getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<const FAdaptiveMerging::AMPartitionManager&>(dynamic_cast<FAdaptiveMerging*>(copy)->getMemoryManager()).getPartitionsRef().size(), (startingEntries + (partitionSize / recordSize) - 1) / (partitionSize / recordSize));

    FAdaptiveMerging copy2(*copy);

    EXPECT_EQ(std::string(copy2.getName()), std::string("FlaggedAdaptiveMerging"));
    ...
    EXPECT_EQ(dynamic_cast<FAdaptiveMerging*>(&copy2)->getMemoryManager().getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<const FAdaptiveMerging::AMPartitionManager&>(dynamic_cast<FAdaptiveMerging*>(&copy2)->getMemoryManager()).getPartitionsRef().size(), (startingEntries + (partitionSize / recordSize) - 1) / (partitionSize / recordSize));

    FAdaptiveMerging copy3;
    copy3 = copy2;

    EXPECT_EQ(std::string(copy3.getName()), std::string("FlaggedAdaptiveMerging"));
    ...
    EXPECT_EQ(dynamic_cast<FAdaptiveMerging*>(&copy3)->getMemoryManager().getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<const FAdaptiveMerging::AMPartitionManager&>(dynamic_cast<FAdaptiveMerging*>(&copy3)->getMemoryManager()).getPartitionsRef().size(), (startingEntries + (partitionSize / recordSize) - 1) / (partitionSize / recordSize));
...
GTEST_TEST(fAdaptiveMerging, moveSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    FAdaptiveMerging* copy = new FAdaptiveMerging(std::move(*dynamic_cast<FAdaptiveMerging*>(am)));
    ...
    FAdaptiveMerging copy2(std::move(*copy));
...
GTEST_TEST(fAdaptiveMerging, insertSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(fAdaptiveMerging, bulkloadSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(fAdaptiveMerging, singleFindPointSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    EXPECT_EQ(dynamic_cast<const FAdaptiveMerging::AMPartitionManager&>(dynamic_cast<FAdaptiveMerging*>(am)->getMemoryManager()).getPartitionsRef().size(), numPartitions);
    ...
    EXPECT_EQ(am->getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<FAdaptiveMerging*>(am)->getMemoryManager().getNumEntries(), startingEntries - 1);
...
GTEST_TEST(fAdaptiveMerging, singleFindRangeSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(fAdaptiveMerging, singleBigFindRangeSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    EXPECT_EQ(am->getNumEntries(), startingEntries);
    EXPECT_EQ(dynamic_cast<FAdaptiveMerging*>(am)->getMemoryManager().getNumEntries(), startingEntries - numOperations);
    EXPECT_EQ(index->getNumEntries(), numOperations);
    ...
    EXPECT_GE(am->getCounter(IndexCounters::INDEX_AM_COUNTER_RO_LOADING_TOTAL_TIME).second, minPagesForEntries* rReadTime);

    EXPECT_GE(am->getCounter(IndexCounters::INDEX_AM_COUNTER_RO_INVALIDATION_TOTAL_OPERATIONS).second, 1);
    EXPECT_EQ(am->getCounter(IndexCounters::INDEX_AM_COUNTER_RO_LOADING_TOTAL_OPERATIONS).second, 1);
...
GTEST_TEST(fAdaptiveMerging, fullFindPointSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    size_t i = 100;
    while (dynamic_cast<FAdaptiveMerging*>(am)->getMemoryManager().getNumEntries() > 0)
    {
        EXPECT_EQ(dynamic_cast<const FAdaptiveMerging::AMPartitionManager&>(dynamic_cast<FAdaptiveMerging*>(am)->getMemoryManager()).getPartitionsRef().size(), numPartitions);
        ...
        EXPECT_EQ(am->getNumEntries(), startingEntries);
        EXPECT_EQ(dynamic_cast<FAdaptiveMerging*>(am)->getMemoryManager().getNumEntries() + index->getNumEntries(), startingEntries);
...
GTEST_TEST(fAdaptiveMerging, fullFindRangeSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(fAdaptiveMerging, singleDeleteSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
...
GTEST_TEST(fAdaptiveMerging, singleBigDeleteSSD)
{
    Disk* disk = new DiskSSD_Samsung840();
    ...
    EXPECT_GT(am->getCounter(IndexCounters::INDEX_AM_COUNTER_RO_INVALIDATION_TOTAL_OPERATIONS).second, 1);
    EXPECT_EQ(am->getCounter(IndexCounters::INDEX_AM_COUNTER_RO_LOADING_TOTAL_OPERATIONS).second, 1);
...
GTEST_TEST(fAdaptiveMerging, interfacePCM)
{
...
GTEST_TEST(fAdaptiveMerging, copyPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, movePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, insertPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, bulkloadPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, singleFindPointPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, singleFindRangePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, singleBigFindRangePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
    ...
    const size_t minPagesForEntries = (numOperations * recordSize + disk->getLowLevelController().getPageSize() - 1) / disk->getLowLevelController().getPageSize();
    EXPECT_GE(am->getCounter(IndexCounters::INDEX_AM_COUNTER_RO_LOADING_TOTAL_TIME).second, minPagesForEntries* readTime);
...
GTEST_TEST(fAdaptiveMerging, fullFindPointPCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, fullFindRangePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, singleDeletePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
...
GTEST_TEST(fAdaptiveMerging, singleBigDeletePCM)
{
    Disk* disk = new DiskPCM_DefaultModel();
    ...
    EXPECT_NEAR(am->getCounter(IndexCounters::INDEX_AM_COUNTER_RO_INVALIDATION_TOTAL_TIME).second, numOperations * (readTime + writeTime), 0.001);
