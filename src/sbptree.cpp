...
size_t SBPTree::calculateNumOfNodes() const noexcept(true)
{
    return calculateNumOfInners() + calculateNumOfLeaves();
}

size_t SBPTree::calculateNumOfInners() const noexcept(true)
{
...
size_t SBPTree::calculateNumOfLeaves() const noexcept(true)
{
    return calculateNumOfLeavesForEntries(numEntries, sizeRecord);
}

size_t SBPTree::calculateNumOfLeavesForEntries(size_t numEntries, size_t entrySize) const noexcept(true)
{
...
size_t SBPTree::calculateHeight() const noexcept(true)
{
    if (numEntries == 0)
        return 0;
...
double SBPTree::findLeaf() noexcept(true)
{
    double time = 0.0;
...
double SBPTree::insertEntriesHelper(size_t numOperations) noexcept(true)
{
    double time = 0.0;
...
double SBPTree::bulkloadEntriesHelper(size_t numEntries) noexcept(true)
{
    double time = 0.0;
...
double SBPTree::deleteEntriesHelper(size_t numOperations) noexcept(true)
{
    double time = 0.0;
...
double SBPTree::findEntriesHelper(size_t numEntries, size_t numOperations) noexcept(true)
{
    double time = 0.0;
...
size_t SBPTree::getHeight() const noexcept(true)
{
    return height;
}

bool SBPTree::isBulkloadSupported() const noexcept(true)
{
    // In oryginal SBPTree bulkload is unsupported, however user can decide if we want to enable this feature during index creation
    return isSpecialBulkloadFeatureOn;
}

double SBPTree::insertEntries(size_t numOperations) noexcept(true)
{
...
double SBPTree::bulkloadEntries(size_t numEntries) noexcept(true)
{
...
double SBPTree::deleteEntries(size_t numOperations) noexcept(true)
{
...
double SBPTree::findPointEntries(size_t numOperations) noexcept(true)
{
...
double SBPTree::findPointEntries(double selectivity, size_t numOperations) noexcept(true)
{
...
double SBPTree::findRangeEntries(size_t numEntries, size_t numOperations) noexcept(true)
{
...
double SBPTree::findRangeEntries(double selectivity, size_t numOperations) noexcept(true)
{
...
std::string SBPTree::toString(bool oneLine) const noexcept(true)
{
    if (oneLine)
        return std::string(std::string("SBPTree {") +
...
std::string SBPTree::toStringFull(bool oneLine) const noexcept(true)
{
    if (oneLine)
        return std::string(std::string("SBPTree {") +
                           std::string(" .sizeKey = ") + std::to_string(sizeKey) +
...
void SBPTree::createTopologyAfterInsert(size_t numEntries) noexcept(true)
{
