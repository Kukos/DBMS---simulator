...
double FBDSM::findKey() noexcept(true)
{
    double time = 0.0;
...
double FBDSM::insertEntriesHelper(size_t numOperations) noexcept(true)
{
    double time = 0.0;
    ...
    for (size_t j = 0; j < columnsSize.size(); ++j)
    {
        ...
        time += disk->writeBytes(addr, columnsSize[j] * entriesInInsertBuffer);
        time += disk->flushCache();
    }
...
double FBDSM::bulkloadEntriesHelper(size_t numEntries) noexcept(true)
{
    double time = 0.0;
...
double FBDSM::deleteEntriesHelper(size_t numOperations) noexcept(true)
{
    double time = 0.0;
    ...
    const size_t deleteFromBuffer = std::min(entriesToDelete, entriesInInsertBuffer);
    const size_t deleteFromTree = entriesToDelete - deleteFromBuffer;
    ...
    for (size_t i = 0; i < deleteFromTree; ++i)
    {
        time += findKey();
...
double FBDSM::findEntriesHelper(const std::vector<size_t>& columnsToFetch, size_t numEntries, size_t numOperations) noexcept(true)
{
    double time = 0.0;
...
std::string FBDSM::toString(bool oneLine) const noexcept(true)
{
    ...
    if (oneLine)
        return std::string(std::string("FBDSM {") +
...
std::string FBDSM::toStringFull(bool oneLine) const noexcept(true)
{
    auto buildStringFromVector = [](const std::string &accumulator, const size_t &columnSize)
    {
        return accumulator.empty() ? std::to_string(columnSize) : accumulator + "," + std::to_string(columnSize);
...
bool FBDSM::isBulkloadSupported() const noexcept(true)
{
    return true;
}

double FBDSM::insertEntries(size_t numOperations) noexcept(true)
{
...
double FBDSM::bulkloadEntries(size_t numEntries) noexcept(true)
{
...
double FBDSM::deleteEntries(size_t numOperations) noexcept(true)
{
...
double FBDSM::findPointEntries(const std::vector<size_t>& columnsToFetch, size_t numOperations) noexcept(true)
{
...
double FBDSM::findPointEntries(const std::vector<size_t>& columnsToFetch, double selectivity, size_t numOperations) noexcept(true)
{
...
double FBDSM::findRangeEntries(const std::vector<size_t>& columnsToFetch, size_t numEntries, size_t numOperations) noexcept(true)
{
...
double FBDSM::findRangeEntries(const std::vector<size_t>& columnsToFetch, double selectivity, size_t numOperations) noexcept(true)
{
