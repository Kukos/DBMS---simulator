...
public:
    ...
    private:
        ...
        bool willBeFullAfterMerge(const FDLvl& lvl);

        friend class FDTree;
    public:
        FDLvl(size_t sizeInBytes, size_t recordSize, size_t lvl);
        ...
        /**
         * @brief Created brief snapshot of FDLvl as a string
         *
         * @param[in] oneLine - create string as 1 line or not? By default Yes
         * @return brief edscription of FDLvl
         */
        std::string toString(bool oneLine = true) const noexcept(true);

        /**
         * @brief Created full snapshot of FDLvl as a string
         *
         * @param[in] oneLine - create string as 1 line or not? By default Yes
         * @return Full edscription of FDLvl
         */
        std::string toStringFull(bool oneLine = true) const noexcept(true);

        virtual ~FDLvl() = default;
        FDLvl() = default;
        FDLvl(const FDLvl&) = default;
        FDLvl& operator=(const FDLvl&) = default;
        FDLvl(FDLvl &&) = default;
        FDLvl& operator=(FDLvl &&) = default;
    };
...
public:
    ...
    */
     ...
     */
    FDTree(Disk* disk, size_t sizeKey, size_t sizeData, size_t nodeSize, size_t headTreeSize, size_t lvlRatio);
    ...
    /**
     * @brief Get FDTree Height (number of levels written on disk). When FD has only HeadTree then height = 0
     *
     * @return FD Height
     */
    ...
    /**
     * @brief Get FD Lvl const reference to see lvl snapshot
     *
     * @param[in] lvl - FD LVL, 0 = HeadTree, 1 = first FDLvl written on disk, FD Height = last lvl
     *
     * @return const reference to FDLvl
     */
    const FDTree::FDLvl& getFDLvl(size_t lvl) const noexcept(true);
    ...
    FDTree() = default;
    FDTree(const FDTree&) = default;
    FDTree& operator=(const FDTree&) = default;
    FDTree(FDTree &&) = default;
    FDTree& operator=(FDTree &&) = default;
};
